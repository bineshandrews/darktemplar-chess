...
/* These definitions are copied from the polyglot book format website.  */
...
CZobrist :: CZobrist(CHESSBOARD *cBrd, GAME_STATE *gState, 
                     MOVELIST *mList, MOVE_TURN *mTurn)
{
    zobristKey = 0;
    updateBoardPosition(cBrd, gState, mList, mTurn);
}

CZobrist :: CZobrist(char *fen)
{
    zobristKey = 0;
    updateBoardPosition(fen);
}

void CZobrist :: updateBoardPosition(CHESSBOARD *cBrd, GAME_STATE *gState, 
                                     MOVELIST *mList, MOVE_TURN *mTurn)
{
     UINT8 i, j;
     CHESSPIECE cp, expectedPiece;
     CHESSPIECE_GENERAL cpg;
     UINT8 color;
     BOARD_POSITION dest;
     UINT8 zobristPieces[] = { 0, 
                               0, /* pawn */
                               3, /* Castle */ 
                               1, /* Knight */
                               2, /* Bishop */
...
*/                             
     ...
     for(i = 0; i < MAXY; i++)
     {
         for(j = 0; j < MAXX; j++)
         {
             ...
             color = CHESSPIECE_COLOR(cp);
             
             cBoard[i][j] = zobristPieces[cpg] * 2 + color;
         }
     }
     ...
     /* enp square is calculated differently from fen..
      * enp sq is valid iff there is an opponent pawn to capture.. */
     if(mList->status & MOVE_EN_PASSENT_ACTIVE)
     {
         color = CHESSPIECE_COLOR(cBrd->board[dest.y][dest.x]);
         
         if(color == WHITE)
             expectedPiece = B_PAWN;
         else
             expectedPiece = W_PAWN;
             
          if((dest.x > 0 && cBrd->board[dest.y][dest.x-1] == expectedPiece) ||
             (dest.x < (MAXX-1) && cBrd->board[dest.y][dest.x+1] == expectedPiece))
          {
              enpOn = TRUE;
              enpFile = dest.x;
          }
...
void CZobrist :: updateBoardPosition(const char *fen)
{
    ...
    char c;
    UINT8 r,f,i,j;
    UINT8 black_pawn = 0, white_pawn = 1;
...
*/     
    ...
    while(1)
    {
        ...
        cBoard[r][f++]= strchr(piece_names, c) - piece_names;
    }
    ...
    while(1)
    {
        if(i >= strlen(castle_flags_s)) 
            break;
            
        switch(castle_flags_s[i++])
        {
            case '-' :
                break;
            case 'K' :
    ...
    if(ep_square_s[0] != '-')
    {
        ...
        if(to_move_c == 'b')
        {
            if((f > 0 && cBoard[3][f-1] == black_pawn)||
               (f < 7 && cBoard[3][f+1] == black_pawn))
            {
                enpOn = TRUE;
                enpFile = f;
            }
        ...
        {
            if((f > 0 && cBoard[4][f-1] == white_pawn)||
               (f < 7 && cBoard[4][f+1] == white_pawn))
            {
                enpOn = TRUE;
                enpFile = f;
            }
...
UINT64 CZobrist :: calculateZobristHash()
{
    UINT8 i, j;
    ...
    for(i = 0; i < MAXY; i++)
    {
        for(j = 0; j < MAXX; j++)
        {
            if(cBoard[i][j] < 12) // max value is white king = 11
            {
                zobristKey ^= RandomPiece[64*cBoard[i][j] +  i * 8 + j];
    ...
    for(i = 0; i < 4; i++)
    {
        if(castling[i] == TRUE)
        {
            zobristKey ^= RandomCastle[i];
        }
    }
    
    if(enpOn == TRUE) 
    {
        zobristKey ^= RandomEnPassant[enpFile];
    }
    
    if(turn == MT_WHITE)
    {
        zobristKey ^= RandomTurn[0];
    }
    
    return zobristKey;
}

UINT64 CZobrist :: getZobristHash()
{
    return zobristKey;
}
